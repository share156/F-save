...
if not MONGO_URI:
    print("⚠️ WARNING: MONGO_URI is not set. Data will not be saved permanently!")
...
auth_col = db["authorized_users"] if db is not None else None
settings_col = db["settings"] if db is not None else None
history_col = db["processed_history"] if db is not None else None
...
def load_data():
    ...
    if not db is None:
        s_doc = sessions_col.find_one({"_id": "sessions"})
        USER_SESSIONS = s_doc.get("data", {}) if s_doc else {}
        
        a_doc = auth_col.find_one({"_id": "auth"})
        AUTHORIZED_USERS = a_doc.get("data", {}) if a_doc else {}
        
        set_doc = settings_col.find_one({"_id": "settings"})
        USER_SETTINGS = set_doc.get("data", {}) if set_doc else {}
        
        h_doc = history_col.find_one({"_id": "history"})
        PROCESSED_HISTORY = h_doc.get("data", {}) if h_doc else {}

def save_sessions():
    if sessions_col is not None: sessions_col.update_one({"_id": "sessions"}, {"$set": {"data": USER_SESSIONS}}, upsert=True)

def save_authorized_users():
    if auth_col is not None: auth_col.update_one({"_id": "auth"}, {"$set": {"data": AUTHORIZED_USERS}}, upsert=True)

def save_user_settings():
    if settings_col is not None: settings_col.update_one({"_id": "settings"}, {"$set": {"data": USER_SETTINGS}}, upsert=True)

def save_history():
    if history_col is not None: history_col.update_one({"_id": "history"}, {"$set": {"data": PROCESSED_HISTORY}}, upsert=True)
...
def init_user_config(uid_str):
    if uid_str not in USER_SETTINGS:
        ...
        if "filters" not in USER_SETTINGS[uid_str]: USER_SETTINGS[uid_str]["filters"] = DEFAULT_FILTERS.copy()
        else:
...
async def handle_text_inputs(client: Client, message: Message):
    ...
    if str_uid in USER_STATES and message.chat.type == pyrogram.enums.ChatType.PRIVATE:
        ...
        if state == "SETTING_CAPTION":
            ...
            if text.lower() == "clear": config["filters"][state.split("_")[1].lower()] = []
            else: config["filters"][state.split("_")[1].lower()] = text.replace(".", "").split() if "EXT" in state else [k.strip() for k in text.split(",") if k.strip()]
            save_user_settings(); USER_STATES.pop(str_uid, None)
        ...
        # Login Steps
            ...
            try:
                ...
                USER_SESSIONS[str_uid] = {"api_id": login_data["api_id"], "api_hash": login_data["api_hash"], "session_string": await login_data["client"].export_session_string()}
            ...
            try:
                ...
                USER_SESSIONS[str_uid] = {"api_id": ACTIVE_LOGINS[user_id]["api_id"], "api_hash": ACTIVE_LOGINS[user_id]["api_hash"], "session_string": await ACTIVE_LOGINS[user_id]["client"].export_session_string()}
                save_sessions(); USER_STATES.pop(str_uid, None)
                await message.reply_text("🎉 **2FA Login Successful!**")
            except Exception as e: await message.reply_text(f"❌ Error: `{e}`")
            return

    # --- BATCH LINK PROCESSOR ---
    if not text: return
    ...
    while True:
        ...
        try:
            ...
            if "https://t.me/c/" in link:
                ...
                if msg.document: await bot.send_document(original_msg.chat.id, file, thumb=thumb, caption=final_cap, reply_to_message_id=original_msg.id, progress=progress_cb, progress_args=(status_msg, "📤 Uploading Document", {"start_time": time.time()}, uid))
                elif msg.video: await bot.send_video(original_msg.chat.id, file, thumb=thumb, caption=final_cap, reply_to_message_id=original_msg.id, progress=progress_cb, progress_args=(status_msg, "📤 Uploading Video File", {"start_time": time.time()}, uid))
                elif msg.photo: await bot.send_photo(original_msg.chat.id, file, caption=final_cap, reply_to_message_id=original_msg.id)
            ...
            # PUBLIC CHATS
                ...
                if msg.document: await bot.send_document(original_msg.chat.id, msg.document.file_id, caption=final_cap, reply_to_message_id=original_msg.id)
                elif msg.video: await bot.send_video(original_msg.chat.id, msg.video.file_id, caption=final_cap, reply_to_message_id=original_msg.id)
                elif msg.photo: await bot.send_photo(original_msg.chat.id, msg.photo.file_id, caption=final_cap, reply_to_message_id=original_msg.id)
                elif msg.text: await bot.send_message(original_msg.chat.id, user_config["caption"] if user_config["use_caption"] else msg.text, reply_to_message_id=original_msg.id)
